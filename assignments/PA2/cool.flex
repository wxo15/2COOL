...
%%
 ...
 /*
  *  \n \t \r \v \f = newline, tab, carriage return, vertical tab, formfeed
  */
\n              { curr_lineno++; }
[ \t\r\v\f]+    {} 
...
<STRING>\\.		{
	switch(yytext[1]) {
		...
		case '0' : string_const[string_const_len++] = 0; str_contain_null_char = true; break;
		default  : string_const[string_const_len++] = yytext[1];
	}
}

 /*
  *  Handle "\" + newline without termination
  */
<STRING>\\\n	{ curr_lineno++; }

 /*
  *  Handle newline without termination (error)
  */
<STRING>\n		{
	curr_lineno++;
...
<STRING>\"		{ 
 ...
 /*
  *  Check length after every new character
  */
<STRING>.		{ 
